...
namespace MarketPriceApi.Services.Instruments
{
    public class GetProvidersQuery
    {
        ...
        public GetProvidersQuery(HttpClient httpClient, FintaAuthService authService, IConfiguration configuration)
        {
        ...
        public async Task<List<Provider>> GetProvidersAsync()
        {
            ...
            {
                throw new Exception($"Failed to get providers: {response.StatusCode}");
            }

            var jsonResponse = await response.Content.ReadAsStringAsync();
            var result = JsonSerializer.Deserialize<List<Provider>>(jsonResponse);
